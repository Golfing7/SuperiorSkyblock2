package com.bgsoftware.superiorskyblock.missions.farmharvesting;
...
public class PlantsTracker {
    ...
    public boolean isTracked(Block block) {
        return isTracked(block.getWorld(), block.getX(), block.getY(), block.getZ());
    }

    public boolean isTracked(World world, int x, int y, int z) {
        loadRawData(world);
        PlantsTrackingComponent trackingComponent = this.plantsTracker.get(world.getName());
        return trackingComponent != null && trackingComponent.getPlacer(x, y, z) != null;
    }
